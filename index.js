...
app.get('/estimate',  (req, res)=>{
    pool.query("SELECT * FROM estimate", (error, result, row)=>{          
        if(error){
            res.send({...response, message:'Error Fetching estimates '})
        }else{          
            res.send({...response, message:`estimates fetched successfully`, status:true, data:result})
        }})
})


app.post('/estimate', (req,res)=>{
  const { project_id, product, quantity, price}=req.body;
  pool.query("INSERT INTO `estimate` (`id`, `project_id`, `product`, `quantity`, `price`) VALUES (NULL, '"+project_id+"', '"+product+"', "+quantity+", "+price+");", (error, result, row)=>{
    if(error){
...
app.delete('/estimate/:id',  (req, res)=>{
    pool.query("DELETE  FROM estimate WHERE id='"+req.params.id+"'", (error, result, row)=>{          
        if(error){
            res.send({...response, message:'Error Deleting estimate '})
        }else{          
            res.send({...response, message:`estimate with ${req.params.id} Deleted successfully`, status:true, data:result})
        }})
})
